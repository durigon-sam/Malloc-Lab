...
Block* searchList(size_t reqSize) {
  ...
  // If there is no first block, we need to create one in the malloc function
  if (ptrFreeBlock == NULL){
    return NULL;
  }
  // Go until you reach end of list
  while(ptrFreeBlock != NULL){
    if (ptrFreeBlock->info.size < 0 && ptrFreeBlock->info.size <= checkSize){
      return ptrFreeBlock;
    } 
  ...
  if(ptrFreeBlock == malloc_list_tail){
    return malloc_list_tail;
  }
  return NULL;
}
...
void* mm_malloc(size_t size) {
  ...
  // THIS RUNS IF THIS IS THE FIRST OR LAST BLOCK IN THE HEAP
  if (ptrFreeBlock == NULL){
    // Create the first block of the heap
  ...
  //SPLITTING
  //First check if we are able to split the block
  if (( -(ptrFreeBlock->info.size) + reqSize + sizeof(BlockInfo)) <= sizeof(BlockInfo) ){
    //Make a pointer to where the split block should begin
    //printf("splitting");
    // WHAT IS THE DIFFERENCE BETWEEN REQSIZE AND SIZE
    splitBlock = (Block*) (UNSCALED_POINTER_ADD(ptrFreeBlock, sizeof(BlockInfo) + reqSize));
    //Sets the next block's prev pointer to the split block
    if (next_block(ptrFreeBlock) == NULL){
      malloc_list_tail = splitBlock;
      malloc_list_tail->info.prev = ptrFreeBlock;
    }else{
      splitBlock->info.prev = ptrFreeBlock;
      
    }
    //Set the split block's prev pointer to the newly allocated block
    
    //Set the size field of the split block to include the free space and the metadata
    splitBlock->info.size = (-(ptrFreeBlock->info.size)) + sizeof(BlockInfo) + reqSize;
    //Set the newly allocated block's size as positive.
  ...
  if (ptrFreeBlock == NULL){
    return NULL;
  }else return (UNSCALED_POINTER_ADD(ptrFreeBlock, sizeof(BlockInfo)));
}
